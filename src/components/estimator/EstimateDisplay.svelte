<script lang="ts">
  ...
  $: if ($columnConfigurations && result) {
    console.log(
      "Column configurations changed, updating grid:",
      $columnConfigurations
    );
    prepareGridData(result);
  }
    ...
    try {
      ...
      const formattedResult = {
        estimate: {
          ...
          totalAmount: estimateItems.reduce(
  ...
  function prepareGridData(data) {
    ...
    data.estimate.lineItems.forEach((item, index) => {
      ...
      if (item.data) {
        ...
        if (typeof dataObj === "object" && dataObj !== null) {
          console.log(
            `Adding custom fields for item ${item.id} from data:`,
            dataObj
          );
          Object.entries(dataObj).forEach(([key, value]) => {
  ...
  function saveColumnWidths(columnDetails: {
    [index: number]: { prop: string; size: number };
  }) {
    try {
      ...
      const existingWidths = JSON.parse(
  ...
  async function handleDeleteItem(item) {
    ...
    if (
      confirm(
        `Are you sure you want to delete ${item.description || "this item"}?`
      )
    ) {
      try {
  ...
  async function handleColumnDeleted(event) {
    console.log("Column deleted event received:", event.detail);
    await loadProjectData(projectId);
    toast.success("Data refreshed after column deletion");
  }
  ...
  async function handleCellEdit(editEvent) {
    ...
    try {
      ...
      console.log("Cell edit event processed:", {
        prop,
        model,
        val,
        oldVal,
      ...
      if (prop === "quantity" || prop === "unitPrice") {
        ...
        const quantity =
          prop === "quantity" ? parseFloat(val) : parseFloat(rowData.quantity);
        const unitPrice =
          prop === "unitPrice"
            ? parseFloat(val)
            : parseFloat(rowData.unitPrice);
        ...
        if (editEvent.data && Array.isArray(editEvent.data)) {
          ...
          const dataIndex = editEvent.data.findIndex(
            (item) => item.id === itemId
          );
          if (dataIndex >= 0) {
      ...
      console.log(
        `Mapping frontend property '${prop}' to database column '${dbProp}'`
      );
      ...
      try {
        ...
        if (isCustomColumn && updateData.data) {
          ...
          console.log(
            "Final data structure being sent to database:",
            updateData.data
          );
        }
          ...
          try {
            ...
            if (verifyError) {
              ...
              if (verifyData.data && verifyData.data[prop] === val) {
                ...
                const itemIndex = gridSource.findIndex(
                  (item) => item.id === actualItemId
                );
                if (itemIndex !== -1) {
                ...
                console.error(
                  "❌ Custom column value was NOT saved correctly!"
                );
        ...
        toast.error(
          `Database update failed: ${dbError.message || "Unknown error"}`
        );
...
{#if gridSource.length > 0}
        ...
        <div class="flex items-center gap-4 flex-grow">
          <div>
            <h4 class="font-medium">
              {result.estimate?.title || "Project Estimate"}
            </h4>
          ...
          <div>
            ...
            <div class="flex items-center gap-2 mt-0.5">
              <span class="text-xs text-slate-500"
                >{result.estimate?.currency || "USD"}</span
              >
              <span class="text-base font-semibold"
                >{result.estimate?.totalAmount || 0}</span
              >
            </div>
          </div>
          ...
          <div class="mt-4 flex justify-between items-center">
            <div>
              ...
              <span class="font-bold"
                >{newItem.amount} {result.estimate?.currency || "USD"}</span
              >
            </div>
            <button
              on:click={addNewItem}
  ...
  <div
    class="grid-container flex-1 bg-white rounded-md overflow-hidden mobile-grid-container"
  >
    ...
    <RevoGrid
      ...
      headerTemplate={(h, column) => {
        ...
        headerEl.className =
          "custom-header flex items-center justify-between w-full px-1";
        ...
        buttonEl.className =
          "add-column-btn ml-1 text-blue-500 hover:text-blue-700 focus:outline-none";
